    ...
    return (
        <nav className="border-gray-200 bg-white dark:bg-gray-900">
            ...
            <div className="flex items-center space-x-1 rtl:space-x-reverse">
                ...
                <span className="self-center text-2xl font-semibold whitespace-nowrap text-[var(--secondary)]">Shoppire</span>
            </div>
            ...
            <div className="flex items-center gap-5 max-md:gap-2 md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse relative">
                ...
                <div onClick={()=> document.documentElement.classList.toggle('dark')} className="w-9">
                    <GoSun className="block dark:hidden text- text-xl mr-10 cursor-pointer p-2 w-fit h-fit rounded-lg hover:bg-gray-200 text-yellow-500"/>
                    <GoMoon className=" hidden dark:block text-xl mr-10 cursor-pointer p-2 w-fit h-fit rounded-lg hover:bg-gray-700 text-violet-700"/>
                </div>

                <div className="relative">
                    <CiShoppingCart className="text-gray-900 text-2xl dark:text-white" /> 
                    {/* <p className=" bg-[var(--secondary)] absolute -top-3.5 left-[2.5px] rounded-full h-5 w-5 text-center text-sm">{localStorage.getItem("cart")}</p> */}
                </div>
            ...
            <div className={`items-center justify-between w-full md:flex md:w-auto md:order-1  max-md:absolute max-md:top-19 max-md:bg-white max-md:w-[99%] rounded-md z-20 ${toggleMenuShow? 'flex': 'hidden'}`} id="navbar-user">
                <ul className="flex flex-col font-medium p-4 md:p-0 mt-4 md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0">
                    <li>
                        <NavLink to="/" className={({isActive})=>`py-2 px-3 text-[var(--secondary)] flex items-center gap-2 ${isActive?' ':'text-gray-900'} dark:text-white`}> <SiGooglehome/> Home</NavLink>
                    </li>
                    <li>
                        <NavLink to="collection" className={({isActive})=> `flex items-center py-2 px-3 text-[var(--secondary)] gap-2 ${isActive?'':'text-gray-900'} dark:text-white`}> <BsCollectionFill/> Collection</NavLink>
                    </li>
                    <li className="flex items-center">
                        <NavLink to="about" className={({isActive})=> `flex items-center py-2 px-3 text-[var(--secondary)] gap-2 ${isActive?'':'text-gray-900'} dark:text-white`}> <SiAtlassian/> About</NavLink>
                    </li>
                    <li className="flex items-center">
                        <NavLink to="contact" className={({isActive})=> `flex items-center py-2 px-3 text-[var(--secondary)] gap-2 ${isActive?'':'text-gray-900'} dark:text-white`}> <SiMaildotcom/> Contact</NavLink>
                    </li>
                    {/* <li>
